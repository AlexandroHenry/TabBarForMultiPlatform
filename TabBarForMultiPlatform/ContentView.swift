...
struct Home: View {
    ...
    // Hiding tab Bar...
    init() {
        // UITab bar is not available for macOS...
        #if os(iOS)
    ...
    var body: some View {
        ...
        ZStack(alignment: getDevice() == .iPhone ? .bottom : .leading) {
            ...
            // Since Tab Bar hide option is not available so we cannot use native tab bar in macOS...
            #if os(iOS)
...
struct InsideTabBarItems: View {
    ...
    var body: some View {
        ...
        Group {
            ...
            .padding(.horizontal, getDevice() == .iPhone ? 15 : 0)
